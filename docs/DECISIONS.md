...
## AD2A.1 Lade Humanoid Construction Study (2026-09-18, Technical, Not Director Acceptance)

- Phase 1 audit (no installs, downloads, or activations): MB-Lab, MakeHuman, and Human Generator/MPFB are not present anywhere local (Blender extensions, Program Files, user documents/downloads); Blender 5.2.1 LTS ships no human base mesh (brush/node/studio-light datafiles only, CC0, no humanoids); installed add-ons are MMD Tools (GPL-3.0), Blender Lab MCP (GPL-3.0), Blender-rpc, and ValveSource, none of which generate human bases; existing local humanoid meshes are the unverified-permission MMD Dolls (rejected as a Lade base on license and identity grounds) and the rejected AD2A primitive candidate (preserved untouched, never rigged/integrated).
- Phase 2 selection: local-only Blender-built-in humanoid-base pipeline. Bundled Rigify human meta-rig (v0.6.10, GPL-2.0-or-later, verified headless with 159 bones) serves as proportion reference only with no generate step, so no GPL rig code enters any export; the body is an original continuous Skin-modifier mesh with scripted anatomical shaping plus a project-owned 20-bone test armature. Output artwork is project-authored; all `.blend`/`.glb`/renders stay local and ignored. This was the only local, properly licensed, automation-compatible method found; the alternative was stopping, which the audit did not require.
- Construction (`tools/lade_ad2a1/`, three model iterations, locked): neutral variant 1.86 m (armature + weights preserved) and baked rifle-ready variant 1.8576 m (temporary IK, both hands solved to 0.0 m, manifested 0.01 m); 15 meshes, 21,152 triangles, 16 AD1-family materials, 0 textures, 10 sockets, grounded, forward-axis correct; inspection PASS with 0 failures (0 non-manifold/loose/degenerate, weight surgery verified at 0.0 stray, GLB round-trips parity with armature surviving in neutral).
- Review route `?modelReview=lade` extended with `variant=base|ad2a1-neutral|ad2a1-ready` (default `base` preserves prior behavior), `side-left` view, `silhouette`/`wireframe` shades, and `sockets=1` gating (markers hidden in normal/gameplay captures). Production builds render nothing; normal Grassland untouched. Evidence: 8 committed contact sheets (including rejected-vs-humanoid comparison and Tololo scale), 18 error-free viewer captures, 120-frame sample avg 6.02 ms / p95 6.5 ms headless, full report in `artifacts/art-direction/ad2a1-lade-model-report.md`.
- Known limits: close-up procedural heritage; rigid pads/coat do not track deep bends; blade reads as antenna in pure silhouette; benign upstream skinned-export validate warning on neutral only. No gameplay, balance, aiming, camera, Tololo, spawning, or lighting change; no integration; no Medisin; no commit/push/deploy. Director acceptance explicitly not recorded.
